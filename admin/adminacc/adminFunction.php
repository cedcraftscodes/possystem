...
function addAdmin()
{
	...
	*/
	...
	else
	{
		...
		$stmt = $conn->prepare("INSERT INTO `tblusers`(`fname`, `mname`, `lname`, `user_name`, `gender`, `birthday`, `pass_word`, `acctype`, `deleted`, `accimg`, `dateadded`, `passchange`) VALUES (:fname, :mname, :lname, :usrn, :gender, :bday, :password , 'admin', 'NO', :img ,:dtt, '0')");
...
function showAdmins()
{
	...
	$admins = $conn->query("SELECT * FROM `tblusers` WHERE deleted='NO' AND `acctype`='admin' AND userid <>".$_SESSION['acc_id']);
